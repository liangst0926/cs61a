...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def select(string):
        new_string = lower(remove_punctuation(string))
        for i in topic:
            if i in split(new_string):
                return True
        return False
    return select
    ...
    if len(typed_words) == 0 or len(reference_words) == 0:
        ...
        for i in range(min(len(reference_words), len(typed_words))):
            if typed_words[i] == reference_words[i]:
                ...
                correct += 0
        return correct / len(reference_words) * 100.0
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    min_word = min(valid_words, key=lambda x: diff_function(x, user_word, limit))
    min_diff = diff_function(min_word, user_word, limit)
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(prompt)):
        if i == len(typed) or typed[1] != prompt[1]:
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(game, j, i) < min_time:
                ...
                min_player = j
        fastest_words[min_player].append(all_words(game)[i])
